  ...
  <motion.div variants={itemVariants} whileHover={{ y: -4, boxShadow: "0 10px 15px -3px rgba(0, 0, 0, 0.1)" }} className={`${gradient} rounded-xl p-6 h-[140px] relative overflow-hidden shadow-sm transition-all duration-300`}>
    ...
    <div className="relative z-10 h-full flex flex-col justify-between">
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col"><span className="text-base font-medium text-white/95">{title}</span><div className="flex items-center gap-1 text-sm text-white/85 mt-1">{TrendIcon && <TrendIcon size={16} />}<span>{subtext}</span></div></div>
      </div>
    </div>
  </motion.div>
);
